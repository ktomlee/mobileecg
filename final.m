...
while k < 13
    ...
    % Saturation (high amplitude) condition and flag setting due to EMG:
    % High amplitude (greater than 2mV for excerpt of continuous 200 msec),
    % considered unacceptable due to EMG, F_EMG set to 1
    while k < 13
        while x < 4901
            count_1 = 1 + x;
            count_2 = 100 + x;  % 200msec (100 samples out of 5000 samples) between count_1 and count_2

            submatrix_D_2 = D(count_1:count_2, k);    % 200msec section of matrix D
            y_max = sum(submatrix_D_2);

            if (y_max >= 100000) % if maximum of submatrix 1mV*100sampels = 100,000
                F_EMG = 1;
...
while i<13
    ...
    check_contact = 0;
    %if (length(peaks{1,i}) < 5) && (check_contact ==3)
    %    F_Contact = 1;
    %end
    
    contact_length = length(peaks{1,i});
    if (contact_length < 5)  %thresholding check: if not enough peaks found by peak finder or min ampl <= 0.125mV = low saturation
        check_contact = check_contact+1;
    end
    if check_contact == 3
        F_Contact = 1;
    end
